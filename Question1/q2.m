...
for ind=1:length(dTypes)
    ...
    [D.(dTypes{ind}).x(:,~D.(dTypes{ind}).labels),D.(dTypes{ind}).dist(:,~D.(dTypes{ind}).labels)] = randGMM(D.(dTypes{ind}).N0,w0,mu0,Sigma0);
    [D.(dTypes{ind}).x(:,D.(dTypes{ind}).labels),D.(dTypes{ind}).dist(:,D.(dTypes{ind}).labels)]=randGMM(D.(dTypes{ind}).N1,w1,mu1,Sigma1);
    subplot(2,2,ind);
    plot(D.(dTypes{ind}).x(1,~D.(dTypes{ind}).labels),D.(dTypes{ind}).x(2,~D.(dTypes{ind}).labels),'g.','DisplayName','Class 0');
    hold all;
    plot(D.(dTypes{ind}).x(1,D.(dTypes{ind}).labels),D.(dTypes{ind}).x(2,D.(dTypes{ind}).labels),'r.','DisplayName','Class 1');
    grid on;
    xlabel('x1');ylabel('x2');
...
px0=evalGMM(D.d10k.x,w0,mu0,Sigma0);
px1=evalGMM(D.d10k.x,w1,mu1,Sigma1);
...
fprintf('lixiang: Gamma=%1.2f, Error=%1.2f%%\n',(p(1)/p(2)),100*pFE_ideal);
...
plotERMContours(D.d10k.x,w0,mu0,Sigma0,w1,mu1,Sigma1,logGamma_ideal);
end
...
for ind=1:length(dTypes)-1
 %Estimate Parameters using matlab built in function
    ...
    plotContours(D.(dTypes{ind}).x,D.(dTypes{ind}).DMM_Est0.ComponentProportion,D.(dTypes{ind}).DMM_Est0.mu,D.(dTypes{ind}).DMM_Est0.Sigma);
    ...
    %Generate vector of gammas for parametric sweep
    logGamma=[min(discScore)-eps sort(discScore)+eps]; 
    prob=CalcProb(discScore,logGamma,D.d10k.labels,D.d10k.N0,D.d10k.N1,D.(dTypes{ind}).phat);
...
options=optimset('MaxFunEvals',3000,'MaxIter',100000);
for ind=1:length(dTypes)
    ...
    quad.x=[ones(1,D.(dTypes{ind}).N); D.(dTypes{ind}).x;D.(dTypes{ind}).x(1,:).^2;D.(dTypes{ind}).x(1,:).*D.(dTypes{ind}).x(2,:);D.(dTypes{ind}).x(2,:).^2];
...
hold on;
plot(logGamma_ideal,pFE_ideal,'og','DisplayName','Calculated Min.Error','LineWidth',2);
xlabel('Gamma');
